...
// trait for implementing different dithering algorithms
pub trait Dither {
    fn dither<P, Container>(
        &self,
        image_in: &ImageBuffer<P, Container>,
        image_out: &mut ImageBuffer<P, Container>,
    ) where
        P: Pixel<Subpixel = u8>,
...
fn threshold<P, T, Container>(
    ...
    Container: DerefMut,
{
    assert!(n >= 2.0, " n was < 2");
    for (&src_pix, dest_pix) in image_in.pixels().zip(image_out.pixels_mut()) {
        *dest_pix = map_pixel(src_pix, n);
    }
}

fn map_pixel<P,T>(mut src_pix: P, n: f32) -> P
where
    P: Pixel<Subpixel = T>,
    T: Primitive + Into<f32>,
{
    for channel in src_pix.channels_mut() {
        let float_color = (n * (*channel).into() / 255.0).round();
        let color = (float_color * 255.0 / n).round();
        *channel = T::from(color).unwrap();
    }
    src_pix.clone()
}
